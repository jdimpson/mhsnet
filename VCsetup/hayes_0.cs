/*
**	Call script to call a host via a Hayes compatible modem (called by "call.modem_0").
**
**	This script calls "hayes_1.cs" once for each modem specified.
**
**	Imports:
...
**		modems		modem(s) attached to device(s) (<type>@<device>[|...])
...
_0_nextmodem:						/* MAIN LOOP STARTS */
	...
	shift type "@" modem;			/* Have type@device */
	match type UNDEFINED		_0_paramerratt2;
	match modem UNDEFINED		_0_paramerratt1;
	set dialset dialstrings;		/* Now set for hayes_1 */
	call "_call/hayes_1.cs";		/* Open modem */
...
_0_paramerr:
	fail "missing some of:\n"
		...
		"\t-D \"modems=<modem type>@<device pathname>[|...]\""
		;
